...
# Configurazione Consumer (AlertSystem)
consumer_config = {
    'bootstrap.servers': KAFKA_HOST,
    'group.id': 'alert_system_group',
    'auto.offset.reset': 'latest',
    'enable.auto.commit': False,  # Gestione manuale per maggiore controllo
}

# Configurazione Producer (AlertSystem)
producer_config = {
    'bootstrap.servers': KAFKA_HOST,
    'acks': 'all',
    'retries': 3,
...
def start_alert_system():
    ...
    try:
        while True:
            # Polling con timeout (come nell'esempio)
            ...
            if msg.error():
                if msg.error().code() == KafkaException._PARTITION_EOF:
                    # Fine della partizione - non è un errore critico
                    continue
                else:
